...
function init(){
  ...
  renderAll();
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('service-worker.js?v=8').then(r => r.update()).catch(()=>{});
}
...
function renderRoutinePresets(){
  ...
  if(!el) return;
  el.innerHTML = ['Custom', ...presetGroups].map(g => `<button class="preset" type="button" data-preset="${esc(g)}">${esc(g)}</button>`).join('');
...
function applyRoutinePreset(group){
  selected = [];
  if(group !== 'Custom'){
    selected = exercises.filter(e => e.group === group).map(e => e.name);
    $('routineName').value = `${group} day`;
    $('routineFilter').value = group;
  } else {
    $('routineName').value = '';
...
function saveRoutine(){
  ...
  selected = []; $('routineName').value = ''; save(); renderAll(); showTab('routineLibrary'); alert('Routine saved to library.');
}
