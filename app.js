const exercises = [
  ...
  ,{name:'Hip Circles',group:'Yoga',equipment:'None',difficulty:'Beginner',notes:'Move slowly and keep the motion pain-free.',muscles:'Hip flexors, glutes, hip rotators, core stability',steps:'Stand with support or start on hands and knees. Make slow circles with one hip, keeping control through the whole range. Switch directions and repeat on the other side.'}
  ,{name:'Figure-4 Stretch',group:'Yoga',equipment:'None',difficulty:'Beginner',notes:'Keep the stretch gentle; do not force the knee.',muscles:'Glutes, piriformis, outer hips, low back',steps:'Lie on your back and cross one ankle over the opposite thigh. Pull the uncrossed leg toward your chest until you feel a stretch in the outer hip. Hold, breathe, and switch sides.'}
  ,{name:'Knee-to-Chest',group:'Yoga',equipment:'None',difficulty:'Beginner',notes:'Relax your shoulders and breathe into the stretch.',muscles:'Low back, glutes, hip extensors',steps:'Lie on your back and gently pull one knee toward your chest. Keep the other leg bent or extended, whichever feels best. Hold, breathe, and switch sides.'}
  ,{name:'Seated Forward Fold',group:'Yoga',equipment:'None',difficulty:'Beginner',notes:'Hinge gently; do not yank yourself forward.',muscles:'Hamstrings, calves, low back',steps:'Sit with legs extended or slightly bent. Reach forward from your hips and let your torso fold only as far as comfortable. Hold and breathe slowly.'}
  ,{name:'Relaxed Breathing',group:'Yoga',equipment:'None',difficulty:'Beginner',notes:'Slow nasal breathing; let your body settle.',muscles:'Diaphragm, nervous system recovery, core relaxation',steps:'Sit or lie comfortably. Inhale slowly through your nose, let your ribs expand, then exhale slowly. Keep your jaw and shoulders relaxed.'}

];

// Keep exercise lists predictable and easy to scan.
...
function exerciseCard(e, withAdd=false){
  return `<div class="card exercise-card" role="button" tabindex="0" data-detail="${esc(e.name)}">
    ...
    ${withAdd ? `<button class="add ${selected.includes(e.name) ? 'is-added' : ''}" type="button" data-add="${esc(e.name)}">${selected.includes(e.name) ? 'Added' : 'Add'}</button>` : ''}
  </div>`;
}
...
function addExercise(name){ if(!selected.includes(name)) selected.push(name); renderSelected(); renderRoutineExercises(); }
function removeSelected(name){ selected = selected.filter(x => x !== name); renderSelected(); renderRoutineExercises(); }
