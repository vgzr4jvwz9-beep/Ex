...
function init(){
  ...
  renderAll();
  if ('serviceWorker' in navigator) navigator.serviceWorker.register('service-worker.js?v=10').then(r => r.update()).catch(()=>{});
}
...
function exerciseCard(e, withAdd=false){
  return `<div class="card exercise-card" role="button" tabindex="0" data-detail="${esc(e.name)}">
    <div class="mini-visual">${exerciseVisual(e.visual, true)}</div>
    <h3>${esc(e.name)}</h3>
...
function openExerciseDetail(name){
  ...
  $('exerciseModalBody').innerHTML = `
    <div class="detail-visual">${exerciseVisual(e.visual)}</div>
    <h2>${esc(e.name)}</h2>
